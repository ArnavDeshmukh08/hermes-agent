...
## Session 2026-06-20 — Jack self-management: self-config + intents + calendar day-name fix + weather/news (BUILT + DEPLOYED)
Orchestrated build (verified SHARED_CONTEXT myself → 4 Sonnet implementers in parallel, disjoint file ownership → 1 follow-up fix → I ran tests/lint/security/deploy directly). **Verified the brief's premises first and corrected three before spawning anything** — same discipline as the prior session paid off again:
- **Config store (the big one):** brief said the self-config tool edits `~/.hermes/config.yaml` flat `JACK_*` keys. Reality = those are **env vars loaded from `~/.hermes/.env`** via `_load_env()`; `config.yaml` is the framework config and ignores `JACK_*`. Retargeted the whole tool to `.env`.
- **Calendar date bug location:** brief said `integrations/calendar.py`. Real cause = `reminders/parser.py` had **NO day-of-week handling at all** (only tomorrow/today/"at X"/"every weekday"), so "Tuesday at 3pm" dropped the day → resolved to today/tomorrow. The confirmation string lives in `handler.py:343`, not calendar.py.
- **Spec example wrong:** brief claimed "Monday from Saturday → Jun 29". Correct answer is **Jun 22** (the actual next Monday); agent flagged + corrected it.

**Built (all graceful-degradation, mock-only tests, zero network in CI):**
- `jack_tools/self_config.py` (+`jack_tools/__init__.py`) — `JackSelfConfig`: `get/set/restart_service/list_configurable/get_status`. `set()` validates type (time/bool/int+range) → atomic `.env` write (mkstemp+fsync+`os.replace`, re-chmod 0600, preserves other lines) → logs to `~/.hermes/logs/self_config.log` → restarts owning systemd `--user` service → **verifies `is-active` before returning success** (honest failure messages; never claims an unapplied change). Security: key allowlist, service-name allowlist, no `shell=True`, only touches `~/.hermes/`. 40 tests.
- `jack_intent_router.py` + `handler.py` — `self_config` intent (set/list/status); `_parse_config_request` (Groq, json_only) maps friendly names → JACK keys via shared `FRIENDLY_TO_KEY`; `_run_self_config` replies ✅ only on `success is True`. Also applied the calendar confirmation reformat in `_run_calendar`. 22 router + handler tests. (handler.py single-owner to avoid the parallel-edit race with the parser fix.)
- `reminders/parser.py` — Mon–Sun + abbreviations, `this/next/on/coming`, resolves to next future occurrence (same day-name → +7, never today/past), combined with clock-time parsing; 09:00 default for bare days. 11 tests.
- `briefing/weather.py` (wttr.in `?format=j1`, stdlib urllib) + `briefing/news.py` (ddgs across 3 Arnav-topics → Groq filter to 2–3) + `morning.py` `_weather_block`/`_news_block` env-gated, system prompt cap 5→10 lines. 49 tests.

**Verification:** **307 tests pass** (193 → +114), ruff clean, no `shell=True`, modules import, unknown-key rejected, `FRIENDLY_TO_KEY` single source of truth (no drift). One integration bug caught + fixed: the new handler test used deprecated `asyncio.get_event_loop()` and failed only in the full suite (py3.13) → swapped to `asyncio.run()`.

**Committed `472fd48`, pushed origin/main.** **DEPLOYED to VPS** (`hermes@167.233.108.213`, `/home/hermes/.hermes/`): backed up `.env`+4 files to `.backups/deploy_selfconfig_20260620/`; SCP'd 7 prod files (md5-verified match); created `~/.hermes/tools/`; appended `JACK_WEATHER_ENABLED=1`, `JACK_WEATHER_LOCATION=Ichalkaranji, India`, `JACK_NEWS_ENABLED=1` to `.env`. **Pre-restart smoke (venv, correct PYTHONPATH):** all prod modules import; `parse_time` live → Tue@3pm=Jun 23, next Mon=Jun 22, tomorrow=Jun 21, today=Jun 20 (all correct); `get_status`=all-active; **weather live = "27°C and Partly cloudy in Ichalkaranji"** (wttr.in + ddgs + googleapiclient all already installed on the box). Restarted gateway+briefing+reminders → all active, gateway `NRestarts=0`, no tracebacks, router re-installed on DiscordAdapter. `jack-daily-sync` was already inactive pre-deploy (no timer — dormant, not a regression). **Live Discord acceptance tests (A–E) pending operator** — see report.

**BUGFIX same session (`f230806`):** first live test "@Jack change my briefing time to 8am" replied "⚠️ Self-config glitch". Root cause = **package-name collision**: the framework (`python -m hermes_cli.main`) owns its own top-level `tools` package (`~/.hermes/hermes-agent/tools`), imported + cached in `sys.modules` at startup, so the hook's lazy `from tools.self_config import …` resolved to the framework's `tools` (no `self_config` submodule) → `ModuleNotFoundError("No module named 'tools.self_config'")`, caught → "glitch". (The earlier standalone import test passed only because cwd put our `tools/` first.) Diagnosed from the gateway journal (`self_config failed: ModuleNotFoundError`). Fix: renamed `tools/` → **`jack_tools/`** (unique, matches `jack_intent_router`/`jack_memory`/`jack_worker`) — collision impossible; updated 2 handler imports + 9 test refs; full traceback now logged on failure (was repr only). Verified live in the real gateway context (framework `tools` cached first, `jack_tools` still resolves) and end-to-end: "change briefing to 8am" → Groq parse `{JACK_BRIEFING_TIME_IST: 08:00}` → `set()` wrote `.env` → **`self_config.log` created** (`SET …=08:00 (was 09:00)`) → `jack-briefing` restarted+confirmed → restored to 09:00. Removed orphaned `~/.hermes/tools/`. 307 tests, ruff clean.

**BUGFIX 2 same session (`45c7646`):** two phrasings fell through to the conversational brain (capability-denial replies) instead of their operational intent. (1) "add a gym session next Monday at 7pm" — the calendar add-branch required a calendar noun (calendar/appointment/meeting/event); "session" wasn't one. Broadened to also match activity bookings (`session|workout|class|practice|training|rehearsal|lesson`) and bare "`schedule/book a X`", the latter guarded with `(?!reminder|alarm)` so "schedule a reminder" stays a reminder. (2) "change the quantum setting to maximum" — self_config/set required a KNOWN setting noun, so unknown settings never routed. Added a generic pattern (config verb + literal "setting(s)", or "configure …"); unknown settings now route to self_config and get the honest "here's what I can change" list (`_parse_config_request` returns None for unmapped keys → handler lists configurables), not a denial. "change my plan" (no "setting" word) stays conversational. Also strip a leading article in calendar title extraction ("a gym session" → "gym session"). **311 tests** (+4), ruff clean. Verified live on VPS in the real gateway context: gym session → calendar/add, title "gym session", **Mon Jun 22 7:00 PM IST** (next Monday is Jun 22, not the Jun 23 in the brief); quantum → self_config/set. Gateway restarted, NRestarts=0.
