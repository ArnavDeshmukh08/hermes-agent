...
class PromptBudgetTest(unittest.TestCase):
    def test_prompt_stays_under_budget(self):
        ...
        # + react rules + few-shots + constraints) but stay well below system +
        # all 20 turns (~5400 tokens), so the sliding-window trimmer is exercised.
        h = _handler(max_turns=20, token_budget=2000)
        for i in range(20):
